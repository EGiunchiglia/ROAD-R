...
def get_filtered_frames_v2(label_key, final_annots, unique_frames, filtered_gts):
    ...
    for item in unique_frames:
        vid_fname = item.split()
        videoname = vid_fname[0]
        frame_name = vid_fname[1][:-4]
        frame_id = int(frame_name)
        frame = final_annots['db'][videoname]['frames'][str(frame_id)]
        frame_name = '{:05d}'.format(int(frame_id))
        if frame['annotated']>0:
            all_boxes = []
            if 'annos' in frame:
...
class VideoDataset(tutils.data.Dataset):
    ...
    def _make_lists_road(self):
        ...
        self.frame_level_list = frame_level_list
        self.all_classes = []
        agentness_label = ['agent_ness']
        self.all_classes.append(agentness_label)
        for k, name in enumerate(self.label_types):
